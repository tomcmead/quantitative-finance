...
class QuantativeAnalysis:
    ...
    def get_earnings_per_share(self, *tickers:str) -> pd.DataFrame:
        market_data = pd.Series([])
        for ticker in tickers:
            ticker_data = yd.Ticker(ticker).get_financial_data('BasicEPS')
            if type(ticker_data)==str:
                print(f"Error: {ticker} not found and excluded")
            else:
                if market_data.empty:
                    market_data = pd.DataFrame(index=[int(str(ticker_data.asOfDate[i])[0:4]) for i in range(4)],
                                               data=[ticker_data.BasicEPS[i] for i in range(4)],
                                               columns=[ticker]).rename_axis('Year')               
                else:
                    market_data_temp = pd.DataFrame(index=[int(str(ticker_data.asOfDate[i])[0:4]) for i in range(4)],
                                                    data=[ticker_data.BasicEPS[i] for i in range(4)],
    ...
    def get_total_liability(self, *tickers:str) -> pd.DataFrame:
        market_data = pd.Series([])
        for ticker in tickers:
            ticker_data = yd.Ticker(ticker).get_financial_data('TotalLiabilitiesNetMinorityInterest')
            if type(ticker_data)==str:
                print(f"Error: {ticker} not found and excluded")
            else:
                if market_data.empty:
                    market_data = pd.DataFrame(index=[int(str(ticker_data.asOfDate[i])[0:4]) for i in range(4)],
                                               data=[ticker_data.TotalLiabilitiesNetMinorityInterest[i] for i in range(4)],
                                               columns=[ticker]).rename_axis('Year')               
                else:
                    market_data_temp = pd.DataFrame(index=[int(str(ticker_data.asOfDate[i])[0:4]) for i in range(4)],
                                                    data=[ticker_data.TotalLiabilitiesNetMinorityInterest[i] for i in range(4)],
    ...
    def get_free_cash_flow(self, *tickers:str) -> pd.DataFrame:
        market_data = pd.Series([])
        for ticker in tickers:
            ticker_data = yd.Ticker(ticker).get_financial_data('FreeCashFlow')
            if type(ticker_data)==str:
                print(f"Error: {ticker} not found and excluded")
            else:
                if market_data.empty:
                    market_data = pd.DataFrame(index=[int(str(ticker_data.asOfDate[i])[0:4]) for i in range(4)],
                                               data=[ticker_data.FreeCashFlow[i] for i in range(4)],
                                               columns=[ticker]).rename_axis('Year')               
                else:
                    market_data_temp = pd.DataFrame(index=[int(str(ticker_data.asOfDate[i])[0:4]) for i in range(4)],
                                                    data=[ticker_data.FreeCashFlow[i] for i in range(4)],
    ...
    def get_change_in_cash(self, *tickers:str) -> pd.DataFrame:
            market_data = pd.Series([])
            for ticker in tickers:
                ticker_data = yd.Ticker(ticker).get_financial_data('ChangeInCashSupplementalAsReported')
                if type(ticker_data)==str:
                    print(f"Error: {ticker} not found and excluded")
                else:
                    if market_data.empty:
                        market_data = pd.DataFrame(index=[int(str(ticker_data.asOfDate[i])[0:4]) for i in range(4)],
                                                data=[ticker_data.ChangeInCashSupplementalAsReported[i] for i in range(4)],
                                                columns=[ticker]).rename_axis('Year')               
                    else:
                        market_data_temp = pd.DataFrame(index=[int(str(ticker_data.asOfDate[i])[0:4]) for i in range(4)],
                                                        data=[ticker_data.ChangeInCashSupplementalAsReported[i] for i in range(4)],
